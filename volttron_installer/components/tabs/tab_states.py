...
class HostTab(rx.State):
    ...
    _committed_hosts = [tab_content.host_entry.id for tab_content in list_of_host_tab_content if tab_content.committed]
    ...
    def append_new_content(self):
        unique_tab_content_uid = f"br{len(self.list_of_host_tab_content)}"
        new_host_entry = HostEntry(id="", ansible_user="", ansible_host="")
    ...
    def commit_host(self, passed_tab_content_id: str):
        ...
        yield rx.toast.success("Agent Saved!")
...
class AgentSetupTab(rx.State):
    ...
    _committed_hosts = [tab_content.agent_entry.identity for tab_content in list_of_agent_tab_content if tab_content.committed]
    ...
    def append_new_content(self):
        ...
        new_agent_entry = AgentDefinition(identity="")
        # print(f"ok brooo this is what i got and stuff:\n{new_host_entry.to_dict()}")
...
class ConfigStoreTemplatesTabContent(rx.Base):
    tab_content_id: str
    template_entry: ConfigStoreEntry
    # agent_entry: BaseAgentContent

    # I want to use a method to like this to compare individual fields. but this implementation doesn't work sadly
    original_template_entry: dict
    ...
    def has_uncaught_changes(self) -> bool:
        current_agent_entry = self.template_entry.to_dict()
        result = current_agent_entry != self.original_template_entry
        return result


class ConfigStoreTemplatesTab(rx.State):
    list_of_agent_tab_content: list[ConfigStoreTemplatesTabContent] = []
    selected_id: str = ""

    _committed_hosts = [tab_content.template_entry.name for tab_content in list_of_agent_tab_content if tab_content.committed]
    ...
    def update_agent_detail(self, agent_tab_content_id, field: AGENT_ENTRY_FIELDS, value: str):
        for i in self.list_of_agent_tab_content:
            if i.tab_content_id == agent_tab_content_id:
                i.uncaught = i.has_uncaught_changes()
                setattr(i.template_entry, field, value)
                # print("uncaught??:", i.has_uncaught_changes())
    ...
    def commit_agent(self, passed_tab_content_id: str):
        for tab_content in self.list_of_agent_tab_content:
            if tab_content.tab_content_id == passed_tab_content_id:
                working_content = tab_content
                working_content.original_template_entry = tab_content.template_entry.to_dict()
                break
        working_content.committed = True
        working_content.uncaught = False
        yield rx.toast.success("Template Saved!")
