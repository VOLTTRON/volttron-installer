...
def render_devices():
    ...
    with devices_container:
        ...
        if not state.discovered_devices:
            ...
            table = ui.table(columns=columns, rows=rows, row_key='id').classes('w-full bg-[#1a1a20]').props('flat bordered dark')
...
def render_dialog_content():
    ...
    with ui.column().classes('w-full gap-4'):
        # Dialog Header
        with ui.row().classes('w-full justify-between items-center pb-4 border-b border-gray-700'):
            with ui.column():
                ui.label(f"Browse Objects: {dev_name}").style('font-size: 1.5rem; font-weight: bold; color: #f3f4f6;')
                ui.label(f"ID: {dev_id} | Address: {dev_addr}").style('font-size: 0.85rem; color: #9ca3af;')
            ui.button(icon='close', on_click=object_dialog.close).props('flat round color="white"')
        ...
        if state.objects_loading:
            with ui.column().classes('w-full py-20 items-center justify-center gap-4'):
                ...
                ui.label("Querying device registers over BACnet...").style('color: #9ca3af; font-size: 1rem;')
        else:
            if not state.objects:
                ...
                table = ui.table(columns=columns, rows=state.objects, selection='multiple', row_key='object_id').classes('w-full bg-[#16161b]').props('flat bordered dark')
                
                # Bottom Actions Row
                with ui.row().classes('w-full justify-between items-center mt-4 flex-nowrap'):
                    # Pagination Controls
                    with ui.row().classes('items-center gap-2'):
                        ui.button(icon='chevron_left', on_click=lambda: fetch_device_objects(dev_addr, dev_id, state.objects_page - 1)).props('flat round color="white"').bind_enabled_from(state, 'objects_page', backward=lambda p: p > 1)
                        ui.label(f"Page {state.objects_page} of {state.objects_total_pages}").style('color: #9ca3af;')
                        ui.button(icon='chevron_right', on_click=lambda: fetch_device_objects(dev_addr, dev_id, state.objects_page + 1)).props('flat round color="white"').bind_enabled_from(state, 'objects_page', backward=lambda p: p < state.objects_total_pages)
                    ...
                    with ui.row().classes('gap-2'):
                        ...
                        ui.button('Close', on_click=object_dialog.close).props('outline color="white"')
...
def render():
    ...
    with ui.dialog() as object_dialog:
        with ui.card().classes('w-full max-w-5xl p-6').style('background: #1e1e24; border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px;'):
